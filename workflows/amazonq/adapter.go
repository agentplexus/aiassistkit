...
// Adapter converts between canonical Workflow and Amazon Q format.
type Adapter struct{}
...
// EntryPointFile returns the entry point filename for Amazon Q.
...
func (a *Adapter) EntryPointDir() string {
	return ".amazonq/rules/spec-workflows"
}

// RuleDetailsDir returns the directory for rule details.
func (a *Adapter) RuleDetailsDir() string {
	return ".amazonq/spec-workflow-details"
}

// Generate creates Amazon Q workflow files from a Workflow.
func (a *Adapter) Generate(workflow *core.Workflow, outputDir string) error {
	// Create rules directory
	rulesDir := filepath.Join(outputDir, a.EntryPointDir())
	if err := os.MkdirAll(rulesDir, core.DefaultDirMode); err != nil {
		return &core.WriteError{Path: rulesDir, Err: err}
	}
	...
	entryPointPath := filepath.Join(rulesDir, a.EntryPointFile())
	...
	if workflow.RuleDetails != "" {
		if err := copyDir(workflow.RuleDetails, filepath.Join(ruleDetailsDir, "rule-details")); err != nil {
			return &core.GenerateError{Platform: "amazonq", Path: ruleDetailsDir, Err: err}
		}
	}
...
// GenerateEntryPoint generates the rule file content for Amazon Q.
